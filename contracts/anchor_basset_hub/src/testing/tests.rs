...
pub fn init<S: Storage, A: Api, Q: Querier>(
    ...
    let register_msg = ExecuteMsg::UpdateConfig {
        ...
        validators_registry_contract: Some(MOCK_REGISTRY_CONTRACT.to_string()),
...
pub fn do_bond(deps: DepsMut, addr: String, amount: Uint128, _validator: Validator) {
    let validators: Vec<RegistryValidator> = deps
        ...
        .query(&QueryRequest::Wasm(WasmQuery::Smart {
            contract_addr: MOCK_REGISTRY_CONTRACT.to_string(),
...
pub fn proper_unbond() {
    ...
    assert_eq!(query_state.total_bond_amount, Uint128::new(10));
    ...
    if res.messages.len() > 2 {
        match &res.messages[0].msg {
            CosmosMsg::Staking(StakingMsg::Undelegate {
                ...
                if val == &validator.address {
                    assert_eq!(amount.amount, Uint128::new(1250))
                }
                if val == &validator2.address {
                    assert_eq!(amount.amount, Uint128::new(1250))
                }
        ...
        match &res.messages[1].msg {
            CosmosMsg::Staking(StakingMsg::Undelegate {
                ...
                if val == &validator.address {
                    assert_eq!(amount.amount, Uint128::new(750))
                }
                if val == &validator2.address {
                    assert_eq!(amount.amount, Uint128::new(750))
                }
